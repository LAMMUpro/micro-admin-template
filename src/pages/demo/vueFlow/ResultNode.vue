...
const operatorSourceConnections = computed(() =>
  getConnectedEdges(sourceConnections.value[0].source).filter(
    (e) => e.source !== sourceConnections.value[0].source
  )
);
...
const result = computed(() => {
  const currResult = operatorData.value.reduce((acc, { data }) => {
    ...
    if (operator) {
      ...
      if (a && b) {
        return (mathFunctions as any)[operator](a, b);
      }
    }
